#include <iostream>
#include <string>
#include <cstring> // Include this for memcmp
...
public:
    ...
    while (memcmp(puzzle, goal, sizeof(puzzle)) != 0) { 
        ...
        int dx = 0, dy = 0;
        if (move == "up" && x > 0) dx = -1;
        else if (move == "down" && x < 2) dx = 1;
        else if (move == "left" && y > 0) dy = -1;
        else if (move == "right" && y < 2) dy = 1;
    ...
    cout << BOLD << "\t\t\t" << string(45, '-') << RESET << endl<<endl<<endl;
}
...
int main() {
    ...
    cout << MAGENTA << BOLD << "\t\t\t\t||" << REVERSED << YELLOW << "\tWelcome To 8-Puzzle Game" << RESET << MAGENTA << BOLD << "\t||" << RESET << endl;
